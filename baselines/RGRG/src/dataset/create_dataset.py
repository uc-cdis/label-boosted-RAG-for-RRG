...
def write_rows_in_new_csv_file(dataset: str, csv_rows: list[list]) -> None:
    ...
    new_csv_file_path += (
        ".csv"
        if not NUM_ROWS_TO_CREATE_IN_NEW_CSV_FILES
        else f"-{NUM_ROWS_TO_CREATE_IN_NEW_CSV_FILES}.csv"
    )
            ...
            for index in range(len(phrases)):
                if phrases[index : index + 8] == "WET READ":
                    ...
                    for curr_index in range(index + 8, len(phrases)):
                        ...
                        if (
                            phrases[curr_index : curr_index + 2] in ["AM", "PM"]
                            or phrases[curr_index : curr_index + 8] == "WET READ"
                        ):
                            break
...
def get_attributes_dict(image_scene_graph: dict, sentence_tokenizer) -> dict[tuple]:
    ...
    for attribute in image_scene_graph["attributes"]:
        ...
        phrases = convert_phrases_to_single_string(
            attribute["phrases"], sentence_tokenizer
        )
...
def get_reference_report(subject_id: str, study_id: str, missing_reports: list[str]):
    ...
    if not os.path.exists(path_to_report):
        shortened_path_to_report = os.path.join(
            f"p{subject_id[:2]}", f"p{subject_id}", f"s{study_id}.txt"
        )
    ...
    if "findings" in section_names:
        ...
        findings_index = (
            len(section_names) - section_names[-1::-1].index("findings") - 1
        )
...
def get_rows(dataset: str, path_csv_file: str, image_ids_to_avoid: set) -> list[list]:
    ...
    with open(path_csv_file) as csv_file:
        ...
        for row in tqdm(csv_reader, total=total_num_rows):
            ...
            if dataset in ["valid", "test"]:
                reference_report = get_reference_report(
                    subject_id, study_id, missing_reports
                )
            ...
            chest_imagenome_scene_graph_file_path = (
                os.path.join(
                    path_chest_imagenome, "silver_dataset", "scene_graph", image_id
                )
                + "_SceneGraph.json"
            )
            ...
            anatomical_region_attributes = get_attributes_dict(
                image_scene_graph, sentence_tokenizer
            )
            ...
            for anatomical_region in ANATOMICAL_REGIONS:
                bbox_coords = region_to_bbox_coordinates_dict.get(
                    anatomical_region, None
                )

                # if there are no bbox coordinates or they are faulty, then don't add them to image information
                if bbox_coords is None or coordinates_faulty(
                    height, width, *bbox_coords
                ):
                    num_faulty_bboxes += 1
                else:
                ...
                bbox_phrase, bbox_is_abnormal = anatomical_region_attributes.get(
                    anatomical_region, ("", False)
                )
            ...
            if dataset == "train" or (
                dataset in ["valid", "test"] and num_regions == 29
            ):
                if dataset in ["valid", "test"]:
            ...
            if (
                ...
                and num_rows_created >= NUM_ROWS_TO_CREATE_IN_NEW_CSV_FILES
            ):
                break

    write_stats_to_log_file(
        dataset,
        num_images_ignored_or_avoided,
        missing_images,
        missing_reports,
        num_faulty_bboxes,
        num_images_without_29_regions,
    )
...
def create_new_csv_file(
    dataset: str, path_csv_file: str, image_ids_to_avoid: set
) -> None:
...
def create_new_csv_files(csv_files_dict, image_ids_to_avoid):
    if os.path.exists(path_full_dataset):
        ...
        log.error(
            "Delete dataset folder or rename variable path_full_dataset in src/path_datasets_and_weights.py before running script to create new folder!"
        )
        return None
...
def get_images_to_avoid():
    path_to_images_to_avoid = os.path.join(
        path_chest_imagenome, "silver_dataset", "splits", "images_to_avoid.csv"
    )
...
def get_train_val_test_csv_files():
    ...
    path_to_splits_folder = os.path.join(
        path_chest_imagenome, "silver_dataset", "splits"
    )
    return {
        dataset: os.path.join(path_to_splits_folder, dataset) + ".csv"
