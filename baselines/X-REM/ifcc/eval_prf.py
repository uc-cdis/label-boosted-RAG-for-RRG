...
def main(args):
    ...
    for c in CONDITIONS:
        if c in trues and c in preds:
            ...
            pr, rc, fb1, _ = precision_recall_fscore_support(
                trues[c], preds[c], labels=pos, average=avg
            )
            prs.append(pr)
            rcs.append(rc)
            fb1s.append(fb1)
    ...
    for c in [
        ...
        preds5 += preds[c]
    pr, rc, fb1, _ = precision_recall_fscore_support(
        trues5, preds5, labels=pos, average=avg
    )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Treat uncertain as an independent class",
    )
    ...
    for impression in impressions:
        ...
        if tokenized_imp:  # not an empty report
            ...
            new_impressions.append(
                torch.LongTensor([tokenizer.cls_token_id, tokenizer.sep_token_id])
            )
