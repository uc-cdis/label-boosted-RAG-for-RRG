...
class CustomCollator:
    ...
    def __call__(self, batch: list[dict[str]]):
        ...
        bbox_phrase_exists_size = batch[0][
            "bbox_phrase_exists"
        ].size()  # should be torch.Size([29])
        region_has_sentence = torch.empty(
            size=(len(batch), *bbox_phrase_exists_size), dtype=torch.bool
        )

        # allocate an empty tensor region_is_abnormal that will store all bbox_is_abnormal tensors of the batch
        bbox_is_abnormal_size = batch[0][
            "bbox_is_abnormal"
        ].size()  # should be torch.Size([29])
        region_is_abnormal = torch.empty(
            size=(len(batch), *bbox_is_abnormal_size), dtype=torch.bool
        )
        ...
        if self.pretrain_without_lm_model:
            ...
            dict_with_ii_and_am = (
                self.transform_to_dict_with_inputs_ids_and_attention_masks(batch)
            )
            ...
            dict_with_ii_and_am = self.tokenizer.pad(
                dict_with_ii_and_am, padding="longest", return_tensors="pt"
            )
