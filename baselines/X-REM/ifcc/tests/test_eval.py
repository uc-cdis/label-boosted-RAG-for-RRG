...
class TestEntityMatcher(unittest.TestCase):
    ...
    def test_score(self):
        rs = self.matcher.score(
            ["1", "2"], ["No pleural effusion.", "Normal heart size."]
        )
...
class TestRecoverWords(unittest.TestCase):
    def setUp(self):
        word_idxs = {
            "__PAD__": PreEmb.INDEX_PAD,
            "__START__": PreEmb.INDEX_START,
            "__UNK__": PreEmb.INDEX_UNKNOWN,
            PreEmb.TOKEN_EOS: PreEmb.INDEX_EOS,
            "Hello": 4,
            "world": 5,
            "!": 6,
            "NLP": 7,
