...
from clinicgen.models.transformer import (
...
class Models:
    ...
    def get_model(
        ...
        device="gpu",
        parallel_sent=True,
        cnnrnnrnn_topic_state=False,
        cnnrnnrnn_simple_proj=False,
        sat_lstm_dim=1000,
        trans_image_pe=True,
        trans_layers=6,
        trans_enc_layers=None,
        ...
        if name == "cnnrnnrnn":
            model = CNNRNNRNN(
                ...
                topic_as_state=cnnrnnrnn_topic_state,
                simple_proj=cnnrnnrnn_simple_proj,
                teacher_forcing=teacher_forcing,
                parallel_sent=parallel_sent,
                view_position=view_position,
                image_model=image_model,
                image_pretrained=image_pretrained,
                finetune_image=finetune_image,
                image_finetune_epoch=image_finetune_epoch,
                rl_opts=rl_opts,
                word_idxs=word_idxs,
                device=device,
                verbose=verbose,
            ...
            model = M2Transformer(
                ...
                max_word=max_word,
                multi_image=multi_image,
                layer_norm=trans_layer_norm,
                num_enc_layers=trans_enc_layers,
                num_dec_layers=trans_layers,
                num_memory=m2_memory,
                teacher_forcing=teacher_forcing,
                image_model=image_model,
                image_pretrained=image_pretrained,
                finetune_image=finetune_image,
            ...
            model = TransformerSimpleCaptioner(
                embeddings,
                feat_dim=hidden_size,
                max_word=max_word,
                multi_image=multi_image,
...
class RLOptions:
    def __init__(
        ...
        self.weights = (
            list(map(lambda v: float(v), weights.split(",")))
        ...
        self.rl_train = False
        self.bleu, self.rouge, self.cider, self.spice, self.bert_score = (
            False,
            False,
            False,
            False,
