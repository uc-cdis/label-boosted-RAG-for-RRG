...
class EpochLog:
    ...
    def log_datasets(
        ...
        for split, data_loader in [("val", val_loader), ("test", test_loader)]:
            ...
            scores = ",".join(
                ["%.2f" % results[split][GenEval.EVAL_SCORE][i] for i in metric_idxs]
            )
...
class FileLogger:
    ...
    def __init__(
        ...
        for split in self.splits:
            fn = (
                "{0}-{1}.txt".format(split, test)
                if test is not None
                else "{0}.txt".format(split)
            )
    ...
    def load_baseline(self, path, optimizers=False):
        if optimizers:
            ...
            self.load_model(
                path, self.model, None, None, None, None, None, device=self.device
            )
            return -1

    def log(self, epoch, data_n, results, save=True):
        updated = (
            self.updates(results["val"][GenEval.EVAL_SCORE]) if "val" in results else []
        )
        for split in ["val", "test"]:
            if split in results:
                ...
                self.log_samples(
                    ...
                    split,
                    updated,
                    result[GenEval.EVAL_ID],
                    result[GenEval.EVAL_REPORT],
                    result[GenEval.EVAL_SCORE_DETAILED],
                )

        if "val" in results:
            for i, (val_score, test_score) in enumerate(
                zip(
                    results["val"][GenEval.EVAL_SCORE],
                    results["test"][GenEval.EVAL_SCORE],
                )
            ):
                self.summary_writer.add_scalars(
                    self.metrics[i], {"val": val_score, "test": test_score}, epoch + 1
                )
        else:
            for i, test_score in enumerate(results["test"][GenEval.EVAL_SCORE]):
                self.summary_writer.add_scalars(
                    self.metrics[i], {"test": test_score}, epoch + 1
                )
    ...
    def log_scores(self, epoch, data_n, split, scores):
        ...
        if not isinstance(scores, list):
            scores = [scores]
        self.scores[split].write(
            "{0} {1}\n".format(lid, " ".join(map(lambda v: str(v), scores)))
        )
        self.scores[split].flush()

    def log_samples(
        self, epoch, data_n, split, updated, report_ids, samples, scores_detailed
    ):
        sample_scores = []
        ...
        for name in names:
            sp_name = (
                "{0}-{1}".format(split, self.test) if self.test is not None else split
            )
            path = os.path.join(
                self.root, "{0}_{1}_samples.txt.gz".format(sp_name, name)
            )
            with gzip.open(path, "wt", encoding="utf8") as out:
                for rid, sample_score, sample in zip(
                    report_ids, sample_scores, samples
                ):
