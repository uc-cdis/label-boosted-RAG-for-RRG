...
parser.add_argument(
    "--reports_path",
    required=True,
    help="Path to file with radiology reports, e.g. /path/to/mimic-cxr/notes",
)
parser.add_argument(
    "--output_path",
    required=True,
    help="Path to output CSV files, e.g. /path/to/mimic-cxr",
)
...
def main(args):
        ...
        for p in tqdm(p_folders):
            ...
            for s in studies:
                ...
                if idx == -1:
                    ...
                    patient_studies.append([study_id, sections[idx].strip()])

                study_sectioned = [study_id]
                for sn in ("impression", "findings", "last_paragraph", "comparison"):
                    if sn in section_names:
                        idx = list_rindex(section_names, sn)
    ...
    if len(patient_studies) > 0:
        ...
        with open(output_path / "mimic_cxr_sectioned.csv", "w") as fp:
            ...
            # write header
            csvwriter.writerow(
                ["study_id", "impression", "findings", "last_paragraph", "comparison"]
            )
            for row in study_sections:
                csvwriter.writerow(row)

        # write all the reports out to a single file
        with open(output_path / f"mimic_cxr_selected_section.csv", "w") as fp:
            csvwriter = csv.writer(fp)
            # write header
            csvwriter.writerow(["study_id", "selected_section"])
