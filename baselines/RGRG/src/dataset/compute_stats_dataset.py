...
def print_stats_counter_dicts(counter_dict):
    ...
    with open(txt_file_to_log_stats, "a") as f:
        ...
        for bbox_name, count in sorted(
            counter_dict.items(), key=lambda k_v: k_v[1], reverse=True
        ):
...
def compute_stats_for_csv_file(
...
def compute_and_print_stats_for_csv_files(csv_files_dict, image_ids_to_avoid):
    ...
    total_stats["total_bbox_with_phrases_counter_dict"] = defaultdict(
        int
    )  # dict to count how often each of the 29 anatomical regions have phrases
    total_stats["total_outlier_bbox_counter_dict"] = defaultdict(
        int
    )  # dict to count how often each of the outlier regions have phrases
    ...
    for dataset, path_csv_file in csv_files_dict.items():
        ...
        for key, value in stats.items():
            if key not in [
                "bbox_with_phrases_counter_dict",
                "outlier_bbox_counter_dict",
            ]:
                ...
                for (
                    bbox_name,
                    count,
                ) in value.items():  # value is a counter dict in this case
