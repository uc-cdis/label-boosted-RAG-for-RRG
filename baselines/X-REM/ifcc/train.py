...
def main(args):
    ...
    print(
        "Data: train={0}, validation={1}, test={2} (load time {3:.2f}s)".format(
    ...
    scheduler_tfr = (
        StepTFR(args.tfr, args.tfr_step) if args.tfr_step is not None else None
    )
    finetune_image = False if args.lr_img == 0.0 else True
    rl_opts = RLOptions(
        epoch=args.rl_epoch,
        metrics=args.rl_metrics,
        weights=args.rl_weights,
        cider_df=cider_df,
        tfidf=rl_tfidf,
        bert_score=args.bert_score,
        bert_score_penalty=args.bert_score_penalty,
        op=args.rl_op,
        nli=args.nli,
        nli_label=args.nli_label,
        nli_neutral_score=args.nli_neutral_score,
        nli_prf=args.nli_prf,
    ...
    model = Models.get_model(
        ...
        args.max_sent,
        multi_image=args.multi_image,
        multi_merge=args.multi_merge,
        teacher_forcing=scheduler_tfr,
        image_model=args.img_model,
        image_pretrained=args.img_pretrained,
        finetune_image=finetune_image,
        view_position=args.view_position,
        parallel_sent=args.parallel_sent,
        image_finetune_epoch=args.img_finetune_epoch,
        rl_opts=rl_opts,
        word_idxs=word_idxs,
        device=device,
        cnnrnnrnn_topic_state=args.cnnrnnrnn_topic_state,
        cnnrnnrnn_simple_proj=args.cnnrnnrnn_simple_proj,
        sat_lstm_dim=args.sat_lstm_dim,
        trans_image_pe=args.img_pe,
        trans_layers=args.trans_layers,
        trans_enc_layers=args.trans_enc_layers,
        trans_layer_norm=args.trans_layer_norm,
        m2_memory=args.m2_memory,
        tienet_labels=args.tienet_labels,
        verbose=args.verbose,
    )
    if device == "gpu":
        model = model.cuda(0)
    optimizers, schedulers, batch_schedulers = Optimizers.get_optmizers(
        model,
        args.lr,
        args.lr_img,
        args.lr_step,
    ...
    with FileLogger(
        ...
        try:
            ...
            if start_epoch > 0:
                ...
                bep = logger.load_baseline(
                    args.baseline_model, optimizers=args.baseline_optimizers
                )
                print(
                    "Starting from baseline {0} ({1})".format(
                        os.path.basename(args.baseline_model), bep + 1
                    )
                )
                # Run evaluation on the baseline model
                if args.rl_start_eval:
                    pbar_vals = EpochLog.log_datasets(
                        logger,
                        pbar_vals,
                        0,
                        0,
                ...
                with tqdm(total=len(train_loader.dataset)) as pbar:
                    ...
                    for ids, inp, targ, vp in train_loader:
                        ...
                        losses = model.train_step(
                            ...
                            device=device,
                            non_blocking=train_loader.pin_memory,
                            epoch=epoch + 1,
                        )
                        ...
                        if (
                            ...
                            and eval_interval >= args.eval_interval
                        ):
                            pbar_vals = EpochLog.log_datasets(
                                logger,
                                pbar_vals,
                        ...
                        # Progress updates
                        if (
                            args.tqdm_interval is None
                            or tqdm_interval >= args.tqdm_interval
                        ):
                            ...
                            tqdm_interval = (
                                tqdm_interval - args.tqdm_interval
                                if args.tqdm_interval is not None
                                else 0
                            )
                # Epoch end processes
...
def parse_args():
    ...
    parser.add_argument(
        "--a-labels", type=str, default=None, help="AReport validity labels"
    )
    ...
    parser.add_argument(
        "--anatomy", type=str, default=None, help="A specific anatomy to target"
    )
    parser.add_argument(
        "--baseline-model",
        type=str,
        default=None,
        help="A baseline model to start from",
    )
    parser.add_argument(
        "--baseline-optimizers",
        default=False,
        action="store_true",
        help="Use baseline optimizers",
    )
    parser.add_argument("--batch-size", type=int, default=24, help="Batch size")
    parser.add_argument(
        "--batch-size-test", type=int, default=None, help="Batch size (test)"
    )
    parser.add_argument("--beam-size", type=int, default=4, help="Beam size")
    parser.add_argument(
        "--bert-score", type=str, default=None, help="BERTScore model type"
    )
    parser.add_argument(
        "--bert-score-penalty",
        default=False,
        action="store_true",
        help="Add a Gaussian penalty to BERTScore",
    )
    parser.add_argument(
        "--cache-data",
        type=str,
        default=None,
        help="Cache images and texts to memory and disk",
    )
    ...
    parser.add_argument(
        "--cnnrnnrnn-mlp-proj",
        dest="cnnrnnrnn_simple_proj",
        default=True,
        action="store_false",
        help="An MLP visual feature projection for CNNRNNRNN",
    )
    parser.add_argument(
        "--cnnrnnrnn-topic-state",
        default=False,
        action="store_true",
        help="Use topic as an initial word LSTM state",
    )
    parser.add_argument(
        "--corpus",
        type=str,
        default="a",
        choices=["a", "flickr30k", "mimic-cxr", "open-i"],
        help="Corpus name",
    )
    parser.add_argument("--cuda", default=False, action="store_true", help="Use GPU")
    parser.add_argument(
        "--entity-match", type=str, default=None, help="A path to reference entities"
    )
    parser.add_argument(
        "--entity-mode", type=str, default="nli-f", help="Entity match mode"
    )
    parser.add_argument("--epochs", type=int, default=32, help="Epoch num")
    parser.add_argument(
        "--eval-interval", type=int, default=None, help="Evaluation interval"
    )
    parser.add_argument(
        "--exclude-ids", type=str, default=None, help="IDs to exclude from the data"
    )
    parser.add_argument("--hidden-size", type=int, default=512, help="Hidden unit size")
    parser.add_argument(
        "--img-finetune-epoch", type=int, default=None, help="Image fine-tuning epoch"
    )
    parser.add_argument(
        "--img-no-augment",
        dest="img_augment",
        default=True,
        action="store_false",
        help="Do not augment images",
    )
    parser.add_argument(
        "--img-pe",
        default=False,
        action="store_true",
        help="Add positional encodings for images",
    )
    parser.add_argument(
        "--img-trans",
        type=str,
        default="pad",
        choices=["center", "pad"],
        help="Image transformation mode",
    )
    parser.add_argument("--img-model", type=str, default=None, help="An image model")
    parser.add_argument(
        "--img-pretrained", type=str, default=None, help="Pre-trained image model"
    )
    parser.add_argument(
        "--iter-sent",
        dest="parallel_sent",
        default=True,
        action="store_false",
        help="Iteratively process sentences",
    )
    parser.add_argument(
        "--log", type=str, default="all", choices=["all", "best"], help="Log mode"
    )
    parser.add_argument(
        "--log-no-models",
        dest="log_models",
        default=True,
        action="store_false",
        help="Do not log models",
    )
    parser.add_argument("--lr", type=float, default=5e-4, help="Learning rate")
    parser.add_argument(
        "--lr-img", type=float, default=None, help="Learning rate for image"
    )
    parser.add_argument(
        "--lr-scheduler",
        type=str,
        default="linear",
        choices=["linear", "trans"],
        help="A learning rate scheduler",
    )
    parser.add_argument(
        "--lr-step", type=int, default=8, help="Epochs to decay the learning rate"
    )
    parser.add_argument(
        "--m2-memory", type=int, default=40, help="M2 Transformer memory size"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="m2trans",
        choices=["cnnrnnrnn", "kwl", "m2trans", "sat", "tienet", "trans", "trans-s"],
    )
    parser.add_argument(
        "--model-save", type=str, default=None, help="A model save path"
    )
    ...
    parser.add_argument(
        ...
        default="max",
        choices=["att", "max"],
        help="A merge method for multi images",
    )
    parser.add_argument(
        "--nli",
        type=str,
        default=None,
        choices=["mednli", "mednli-rad"],
        help="NLI model type",
    )
    parser.add_argument("--nli-batch", type=int, default=24, help="NLI batch size")
    parser.add_argument(
        "--nli-comp", type=str, default="bert-score", help="NLI comparison method"
    )
    parser.add_argument(
        "--nli-label",
        type=str,
        ...
        help="NLI score label",
    )
    parser.add_argument(
        "--nli-neutral-score",
        type=float,
        default=(1.0 / 3),
        help="An NLI entailment neutral score",
    )
    parser.add_argument(
        "--nli-prf",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of background workers for data loader",
    )
    parser.add_argument(
        "--pin-memory",
        dest="pin_memory",
        default=False,
        action="store_true",
        help="Use pin-memory on data loaders",
    )
    parser.add_argument(
        "--rl-epoch", type=int, default=None, help="Self-critical RL staring epoch"
    )
    parser.add_argument(
        "--rl-metrics", type=str, default=None, help="Self-critical RL reward metrics"
    )
    parser.add_argument(
        "--rl-no-start-eval",
        dest="rl_start_eval",
        default=True,
        action="store_false",
        help="Do not evaluate at start in RL training",
    )
    parser.add_argument(
        "--rl-op", type=str, default="add", choices=["add", "mul"], help="RL operator"
    )
    parser.add_argument(
        "--rl-weights", type=str, default=None, help="A scaling weights for RL training"
    )
    parser.add_argument(
        "--sat-lstm-dim", type=int, default=1000, help="An LSTM dimension for SAT"
    )
    parser.add_argument(
        "--section", type=str, default="findings", help="Report section"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="none",
        choices=["linebreak", "nltk", "none", "stanford"],
        help="Sentence splitter name",
    )
    parser.add_argument(
        "--single-test",
        default=False,
        action="store_true",
        help="Test with a single image setting",
    )
    parser.add_argument(
        "--spice", default=False, action="store_true", help="SPICE evaluation"
    )
    parser.add_argument(
        "--splits", type=str, default=None, help="A path to a file defining splits"
    )
    parser.add_argument(
        "--stanza-download",
        default=False,
        action="store_true",
        help="Download Stanza clinical model",
    )
    parser.add_argument("--tfr", type=float, default=1.0, help="Teacher forcing rate")
    parser.add_argument(
        "--tfr-step", type=int, default=None, help="Teacher forcing step"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="nltk",
        choices=["nltk", "none", "stanford", "whitespace"],
        help="Tokenizer name",
    )
    ...
    parser.add_argument(
        "--trans-layers", type=int, default=1, help="Number of transformer layers"
    )
    parser.add_argument(
        "--trans-layer-no-norm",
        dest="trans_layer_norm",
        default=True,
        action="store_false",
        help="Do not Layer normalize visual features in transformer models",
    )
    parser.add_argument(
        "--verbose", default=False, action="store_true", help="Verbose outputs"
    )
    parser.add_argument(
        "--view-position",
        default=False,
        action="store_true",
        help="Include view position embeddings",
    )
    parser.add_argument(
        "--warmup", type=int, default=2000, help="Warm-up steps for optimizers"
    )
