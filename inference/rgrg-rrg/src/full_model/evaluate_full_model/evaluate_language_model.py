...
def compute_NLG_scores(
    nlg_metrics: list[str],
    gen_sents_or_reports: list[str],
    ref_sents_or_reports: list[str],
) -> dict[str, float]:
    ...
    if "rouge" in nlg_metrics:
        scorers["rouge"] = (
            Rouge()
        )  # this is actually the Rouge-L score, even if the class name only says Rouge
    if "cider" in nlg_metrics:
        scorers["cider"] = (
            Cider()
        )  # this is actually the Cider-D score, even if the class name only says Cider
...
def compute_clinical_efficacy_scores(
    language_model_scores: dict, gen_reports: list[str], ref_reports: list[str]
):
    """
    ...
    def get_chexbert():
        ...
        model = nn.DataParallel(
            model
        )  # needed since weights were saved with nn.DataParallel
    ...
    def compute_micro_average_CE_scores(preds_gen_reports, preds_ref_reports):
        def convert_labels_like_miura(preds_reports: list[list[int]]):
            ...
            preds_reports_converted = [
                [convert_label(label) for label in condition_list]
                for condition_list in preds_reports
            ]
        ...
        for preds_gen_reports_condition, preds_ref_reports_condition, condition in zip(
            preds_gen_reports_converted, preds_ref_reports_converted, CONDITIONS
        ):
            ...
            precision, recall, f1, _ = precision_recall_fscore_support(
                preds_ref_reports_condition,
                preds_gen_reports_condition,
                average="binary",
            )
            acc = accuracy_score(
                preds_ref_reports_condition, preds_gen_reports_condition
            )
        ...
        acc = accuracy_score(
            total_preds_ref_reports_14_conditions, total_preds_gen_reports_14_conditions
        )
        ...
        acc = accuracy_score(
            total_preds_ref_reports_5_conditions, total_preds_gen_reports_5_conditions
        )
    ...
    def compute_example_based_CE_scores(preds_gen_reports, preds_ref_reports):
        ...
        """
        preds_gen_reports_np = np.array(
            preds_gen_reports
        )  # array of shape (14 x num_reports), 14 for 14 conditions
        preds_ref_reports_np = np.array(
            preds_ref_reports
        )  # array of shape (14 x num_reports)
        ...
        tp = np.logical_and(
            preds_gen_reports_np, preds_ref_reports_np
        )  # bool array of shape (14 x num_reports)
        fp = np.logical_and(
            preds_gen_reports_np, ~preds_ref_reports_np
        )  # bool array of shape (14 x num_reports)
        fn = np.logical_and(
        ...
        precision_example = tp_example / (
            tp_example + fp_example
        )  # float array of shape (num_reports)
        recall_example = tp_example / (
            tp_example + fn_example
        )  # float array of shape (num_reports)
        f1_example = (2 * tp_example) / (
            2 * tp_example + fp_example + fn_example
        )  # float array of shape (num_reports)
        acc_example = (tp_example + tn_example) / (
            tp_example + tn_example + fp_example + fn_example
        )  # float array of shape (num_reports)
...
def compute_language_model_scores(gen_and_ref_sentences, gen_and_ref_reports):
    ...
    def compute_sentence_level_scores():
        ...
        def compute_sent_level_meteor_ratio_score(gen_sents, ref_sents):
            ...
            num_generated_sentences_per_image = gen_and_ref_sentences[
            ...
            for num_gen_sents in num_generated_sentences_per_image:
                gen_sents_single_image = gen_sents[
                    curr_index : curr_index + num_gen_sents
                ]
                ref_sents_single_image = ref_sents[
                    curr_index : curr_index + num_gen_sents
                ]
                ...
                gen_sents_single_image_filtered, ref_sents_single_image_filtered = (
                    remove_gen_sents_corresponding_to_empty_ref_sents(
                        gen_sents_single_image, ref_sents_single_image
                    )
                )
            ...
            language_model_scores["all"]["meteor_ratio"] = (
                numerator_meteor_score / denominator_meteor_score
            )

        generated_sents = gen_and_ref_sentences["generated_sentences"]
        generated_sents_normal = gen_and_ref_sentences[
            "generated_sentences_normal_selected_regions"
        ]
        generated_sents_abnormal = gen_and_ref_sentences[
            "generated_sentences_abnormal_selected_regions"
        ]

        reference_sents = gen_and_ref_sentences["reference_sentences"]
        reference_sents_normal = gen_and_ref_sentences[
            "reference_sentences_normal_selected_regions"
        ]
        reference_sents_abnormal = gen_and_ref_sentences[
            "reference_sentences_abnormal_selected_regions"
        ]

        gen_sents_filtered, ref_sents_filtered = (
            remove_gen_sents_corresponding_to_empty_ref_sents(
                generated_sents, reference_sents
            )
        )
        gen_sents_normal_filtered, ref_sents_normal_filtered = (
            remove_gen_sents_corresponding_to_empty_ref_sents(
                generated_sents_normal, reference_sents_normal
            )
        )
        gen_sents_abnormal_filtered, ref_sents_abnormal_filtered = (
            remove_gen_sents_corresponding_to_empty_ref_sents(
                generated_sents_abnormal, reference_sents_abnormal
            )
        )

        compute_sent_level_scores_for_subset(
            "all", gen_sents_filtered, ref_sents_filtered
        )
        compute_sent_level_scores_for_subset(
            "normal", gen_sents_normal_filtered, ref_sents_normal_filtered
        )
        compute_sent_level_scores_for_subset(
            "abnormal", gen_sents_abnormal_filtered, ref_sents_abnormal_filtered
        )
        ...
        for region_index, region_name in enumerate(ANATOMICAL_REGIONS):
            region_generated_sentences = gen_and_ref_sentences[region_index][
                "generated_sentences"
            ]
            region_reference_sentences = gen_and_ref_sentences[region_index][
                "reference_sentences"
            ]

            region_gen_sents_filtered, region_ref_sents_filtered = (
                remove_gen_sents_corresponding_to_empty_ref_sents(
                    region_generated_sentences, region_reference_sentences
                )
            )

            if len(region_gen_sents_filtered) != 0:
                compute_sent_level_scores_for_region(
                    region_name, region_gen_sents_filtered, region_ref_sents_filtered
                )
            else:
    ...
    def compute_report_level_scores():
        ...
        compute_clinical_efficacy_scores(
            language_model_scores, gen_reports, ref_reports
        )
...
def write_sentences_and_reports_to_file(
    ...
    def write_sentences():
        txt_file_name = os.path.join(
            generated_sentences_and_reports_folder_path,
            "generated_sentences",
            f"generated_sentences_step_{overall_steps_taken}",
        )
        txt_file_name_abnormal = os.path.join(
            generated_sentences_and_reports_folder_path,
            "generated_sentences",
            f"generated_abnormal_sentences_step_{overall_steps_taken}",
        )
...
def update_region_set_text(
    ...
    reference_sentence_region = transform_sentence_to_fit_under_image(
        reference_sentence_region
    )
    ...
    if not box_region_selected:
        ...
        generated_sentence_region = transform_sentence_to_fit_under_image(
            generated_sentence_region
        )
        ...
        for num_region_set, region_set in enumerate(regions_sets):
            ...
            for region_index, color in zip(region_indices, region_colors):
                ...
                plot_box(
                    ...
                    clr=color,
                    linestyle="solid",
                    region_detected=box_region_detected,
                )
...
def update_gen_sentences_with_corresponding_regions(
    ...
    for selected_regions_single_image in selected_regions:
        ...
        for region_index, region_selected_bool in enumerate(
            selected_regions_single_image
        ):
        ...
        gen_sentences_with_corresponding_regions.append(
            gen_sents_with_regions_single_image
        )


def update_num_generated_sentences_per_image(
    gen_and_ref_sentences: dict, selected_regions: np.array
):
    """
    ...
    """
    num_gen_sents_per_image = selected_regions.sum(
        axis=1
    ).tolist()  # indices is a list[int] of len(batch_size)
    gen_and_ref_sentences["num_generated_sentences_per_image"].extend(
        num_gen_sents_per_image
    )
...
def get_generated_reports(
            ...
            for j in range(i + 1, len(gen_sents_single_image)):
                ...
                if bert_score_result["f1"][0] > bertscore_threshold:
                    ...
                    if len(gen_sent_1) > len(gen_sent_2):
                        similar_generated_sents_to_be_removed[gen_sent_1].append(
                            gen_sent_2
                        )
                    else:
                        similar_generated_sents_to_be_removed[gen_sent_2].append(
                            gen_sent_1
                        )
    ...
    for selected_regions_single_image in selected_regions:
        ...
        gen_report_single_image, similar_generated_sents_to_be_removed = (
            remove_duplicate_generated_sentences(gen_report_single_image, bert_score)
        )

        generated_reports.append(gen_report_single_image)
        removed_similar_generated_sentences.append(
...
def get_sents_for_normal_abnormal_selected_regions(
    ...
    gen_sents_for_selected_regions = np.asarray(
        generated_sentences_for_selected_regions
    )
    ref_sents_for_selected_regions = np.asarray(
        reference_sentences_for_selected_regions
    )

    gen_sents_for_normal_selected_regions = gen_sents_for_selected_regions[
        ~selected_region_is_abnormal
    ].tolist()
    gen_sents_for_abnormal_selected_regions = gen_sents_for_selected_regions[
        selected_region_is_abnormal
    ].tolist()

    ref_sents_for_normal_selected_regions = ref_sents_for_selected_regions[
        ~selected_region_is_abnormal
    ].tolist()
    ref_sents_for_abnormal_selected_regions = ref_sents_for_selected_regions[
        selected_region_is_abnormal
    ].tolist()
...
def evaluate_language_model(
    ...
    with torch.no_grad():
        for num_batch, batch in tqdm(
            enumerate(val_dl),
            total=NUM_BATCHES_TO_PROCESS_FOR_LANGUAGE_MODEL_EVALUATION,
        ):
            ...
            if output == -1:
                ...
                beam_search_output, selected_regions, detections, class_detected = (
                    output
                )
