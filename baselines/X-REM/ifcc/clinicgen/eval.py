...
from clinicgen.utils import RecoverWords, data_cuda
...
class EntityMatcher:
    ...
    def __init__(
        self, sentences, entities, target_types, mode="exact", batch=48, nli=None
    ):
    ...
    def load_ner(cls):
        config = {
            "tokenize_batch_size": cls.NER_BATCH_SIZE,
            "ner_batch_size": cls.NER_BATCH_SIZE,
        }
        return Pipeline(
            lang="en",
            package="radiology",
    ...
    def score(self, rids, hypos):
        ...
        if self.mode.startswith(self.MODE_NLI):
            ...
            _, _, _, stats = self.nli.sentence_scores_bert_score(
                texts1, texts2, label="all", prf=self.prf
            )
        ...
        for i, rid in enumerate(rids):
                        ...
                        if hypo_nli is not None:
                            if hypo_nli[i][sid] == "neutral":
                                ...
                                if s in hypo_entities:
                                    ...
                                    if (
                                        self.mode == self.MODE_NLI
                                        or self.mode == self.MODE_NLI_ENTAILMENT
                                    ):
            ...
            if self.prf == "p":
                ...
                score_e = (
                    2 * pr_e * rc_e / (pr_e + rc_e)
                    if pr_e > 0.0 and rc_e > 0.0
                    else 0.0
                )
                score_n = (
                    2 * pr_n * rc_n / (pr_n + rc_n)
                    if pr_n > 0.0 and rc_n > 0.0
                    else 0.0
                )
            if self.penalty:
                penalty = np.e ** (
                    -((total_pr - total_rc) ** 2) / (2 * self.PENALTY_SIGMA**2)
                )
                score_e *= penalty
                score_n *= penalty
...
class GenEval:
    ...
    def __init__(
        ...
        spice=False,
        bert_score=None,
        bert_score_penalty=False,
        nli=None,
        nli_compare=None,
        nli_label="entailment",
        nli_neutral_score=(1.0 / 3),
        nli_prf="f",
        nli_batch=16,
        nli_cache=None,
        entity_match=None,
        ...
        self.nli = nli
        nli_compare = (
            nli_compare.split(",") if isinstance(nli_compare, str) else nli_compare
        )
            ...
            for i in range(len(rs1)):
                if isinstance(rs2[i], float) or isinstance(rs2[i], np.float32):
                    ...
                    if isinstance(rs1[i], list):
                        ...
                        raise ValueError(
                            "Unsupported result {0} in index {1}".format(
                                type(rs1[i]).__name__, i
                            )
                        )
        return rs1
    ...
    def full_metrics(cls):
        nli_compare = [
            SimpleNLI.COMPARE_DOC,
            SimpleNLI.COMPARE_BERT_SCORE,
            SimpleNLI.COMPARE_BERT_SCORE_FIX_THRESH,
            SimpleNLI.COMPARE_TFIDF,
            SimpleNLI.COMPARE_ALL,
        ]
        return cls.get_metrics(True, True, True, True, True, True, True, nli_compare)

    @classmethod
    def get_metrics(
        cls, bleu, rouge, cider, spice, bert_score, nli, entity_match, nli_compare
    ):
        m = {}
        if bleu:
            idx = len(m)
    ...
    def eval(self, ids, refs, hypos, tfidf_vectorizer=None, ref_ids=None):
        ...
        if self.nli_model is not None:
            ...
            if SimpleNLI.COMPARE_DOC in comps:
                ...
                nli_scores1 = {
                    SimpleNLI.LABEL_ENTAIL: [],
                    SimpleNLI.LABEL_NEUTRAL: [],
                    SimpleNLI.LABEL_CONTRADICT: [],
                }
                for prob in probs:
            ...
            if SimpleNLI.COMPARE_BERT_SCORE in comps:
                _, _, vs, _ = self.nli_model.sentence_scores(
                    refs_l,
                    hypos_l,
                    SimpleNLI.COMPARE_BERT_SCORE,
                    label=self.nli_label,
                    prf=self.nli_prf,
                )
    ...
    def eval_batch(
        ...
        for i, rid in enumerate(ids):
            ...
            if c >= batch_size:
                s1, s2 = self.eval(
                    ids_set, refs_set, hypos_set, tfidf_vectorizer, ref_ids_set
                )
    ...
    def generate_and_eval(self, data_loader, progress_name=None, batch=False):
        ...
        with torch.no_grad():
            ...
            for rids, inp, targ, vp in data_loader:
                inp = data_cuda(
                    inp, device=self.device, non_blocking=data_loader.pin_memory
                )
                meta = (vp,)
                meta = self.model.meta_cuda(
                    meta, device=self.device, non_blocking=data_loader.pin_memory
                )
        ...
        if batch:
            ...
            scores, scores_detailed = self.eval(
                report_ids, refs, hypos, tfidf_vectorizer
            )
        return {
    ...
    def setup(self):
        if self.nli == self.NLI_MED or self.nli == self.NLI_RAD_AUG:
            ...
            self.nli_model = SimpleNLI(
                model,
                batch=self.nli_batch,
                neutral_score=self.nli_neutral_score,
                nthreads=self.nthreads,
                pin_memory=self.pin_memory,
                bert_score=bert_score,
                sentsplitter=self.sentsplitter,
                cache=self.nli_cache,
                verbose=self.verbose,
            )
        if self.bert_score is not None:
            self.bert_score_model = BERTScorer(
                model_type=self.bert_score,
        ...
        if self.entity_match is not None:
            ...
            sentences, entities = EntityMatcher.load_entities(
                self.entity_match, target_types
            )
