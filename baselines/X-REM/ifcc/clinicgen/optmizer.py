...
class Optimizers:
    ...
    def get_optmizers(
        ...
        if lr_scheduler == cls.TRANSFORMER:
            ...
            schedulers = {}
            batch_schedulers = {
                cls.TEXT: TransformerScheduler(
                    optimizer, d_train, steps_per_epoch, warmup
                )
            }
        else:
            if lr_img is None or lr == lr_img:
                optimizers = {
                    cls.TEXT: Adam(model.parameters(), lr=lr, betas=(beta1, beta2))
                }
                schedulers = {
                    cls.TEXT: StepLR(optimizers[cls.TEXT], lr_step, lr_decay_rate)
                }
            else:
                if lr_img is None:
                    lr_img = lr
                ...
                optimizers = {
                    cls.TEXT: Adam(text_params, lr=lr, betas=(beta1, beta2)),
...
class TransformerScheduler(_LRScheduler):
    ...
    def get_lr(self):
        ...
        step = max(1, step)
        return [
            self.d_train ** (-0.5) * min(step ** (-0.5), step * self.warmup ** (-1.5))
