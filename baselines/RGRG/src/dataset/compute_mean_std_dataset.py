...
def last_and_curr_mean_std_close(
    curr_mean, curr_std, last_mean_values, last_std_values
):
...
def get_mean_std(image_paths) -> tuple([float, float]):
    ...
    for num_image, image_path in enumerate(image_paths, start=1):
        ...
        print(
            f"current mean: {curr_mean:.3f} \tcurrent std: {curr_std:.3f} \tcounter: {counter}"
        )

        if last_and_curr_mean_std_close(
            curr_mean, curr_std, last_mean_values, last_std_values
        ):
            counter += 1
        else:
