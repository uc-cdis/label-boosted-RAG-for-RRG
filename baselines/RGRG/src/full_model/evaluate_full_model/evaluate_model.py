...
def write_all_losses_and_scores_to_tensorboard(
    ...
    def write_obj_detector_scores():
        ...
        for region_, avg_detections_region in zip(
            ...
            writer.add_scalar(
                f"object_detector/num_detected/{region_}",
                avg_detections_region,
                overall_steps_taken,
            )

        for region_, avg_iou_region in zip(anatomical_regions, avg_iou_per_region):
            writer.add_scalar(
                f"object_detector/iou/{region_}", avg_iou_region, overall_steps_taken
            )
    ...
    def write_region_selection_scores():
        for subset in region_selection_scores:
            for metric, score in region_selection_scores[subset].items():
                writer.add_scalar(
                    f"region_select/{subset}/{metric}", score, overall_steps_taken
                )
    ...
    def write_clinical_efficacy_scores(ce_score_dict):
        ...
        for k, v in ce_score_dict.items():
            if (
                ...
                writer.add_scalar(
                    f"language_model/report/CE/{k}", v, overall_steps_taken
                )
            else:
                # k is a condition
                ...
                for metric, score in ce_score_dict[k].items():
                    writer.add_scalar(
                        f"language_model/report/CE/{condition_name}/{metric}",
                        score,
                        overall_steps_taken,
                    )
            ...
            if subset == "region":
                for region_name in language_model_scores["region"]:
                    for metric, score in language_model_scores["region"][
                        ...
                        writer.add_scalar(
                            f"language_model/region/{region_name_underscored}/{metric}",
                            score,
                            overall_steps_taken,
                        )
            else:
                for metric, score in language_model_scores[subset].items():
                    if metric == "CE":
                        ...
                        writer.add_scalar(
                            f"language_model/{subset}/{metric}",
                            score,
                            overall_steps_taken,
                        )
...
def update_region_abnormal_metrics(
    ...
    region_abnormal_scores["precision"](
        detected_predicted_abnormal_regions, detected_region_is_abnormal
    )
    region_abnormal_scores["recall"](
        detected_predicted_abnormal_regions, detected_region_is_abnormal
    )
    region_abnormal_scores["f1"](
        detected_predicted_abnormal_regions, detected_region_is_abnormal
    )


def update_region_selection_metrics(
    ...
    region_selection_scores["all"]["precision"](
        selected_regions.reshape(-1), region_has_sentence.reshape(-1)
    )
    region_selection_scores["all"]["recall"](
        selected_regions.reshape(-1), region_has_sentence.reshape(-1)
    )
    region_selection_scores["all"]["f1"](
        selected_regions.reshape(-1), region_has_sentence.reshape(-1)
    )

    region_selection_scores["normal"]["precision"](
        normal_selected_regions, normal_region_has_sentence
    )
    region_selection_scores["normal"]["recall"](
        normal_selected_regions, normal_region_has_sentence
    )
    region_selection_scores["normal"]["f1"](
        normal_selected_regions, normal_region_has_sentence
    )

    region_selection_scores["abnormal"]["precision"](
        abnormal_selected_regions, abnormal_region_has_sentence
    )
    region_selection_scores["abnormal"]["recall"](
        abnormal_selected_regions, abnormal_region_has_sentence
    )
    region_selection_scores["abnormal"]["f1"](
        abnormal_selected_regions, abnormal_region_has_sentence
    )


def update_object_detector_metrics(
...
def evaluate_model(
    ...
    if total_val_loss < run_params["lowest_val_loss"]:
        ...
        save_path = os.path.join(
            run_params["checkpoints_folder_path"],
            f"checkpoint_val_loss_{total_val_loss:.3f}_overall_steps_{overall_steps_taken}.pt",
        )
