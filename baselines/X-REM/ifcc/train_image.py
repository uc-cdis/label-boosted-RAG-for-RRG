        ...
        if data_loader is not None:
            ...
            if split == "val":
                ...
                for update in updates:
                    save_model(
                        os.path.join(args.out, "model_{0}.dict.gz".format(update)),
                        epoch,
                        model,
                        optimizer,
                        scheduler,
...
def main(args):
    ...
    print(
        "Data: train={0}, validation={1}, test={2} (load time {3:.2f}s)".format(
            len(train_loader.dataset.samples),
            ...
            with tqdm(total=len(train_loader.dataset.samples)) as pbar:
                ...
                for _, inp, targ, _ in train_loader:
                    ...
                    loss_val = model.train_step(
                        inp, targ, optimizer, clip_grad=args.clip_grad, device=device
                    )
                    ...
                    if (
                        ...
                        tqdm_interval -= (
                            args.tqdm_interval if args.tqdm_interval is not None else 0
                        )
            # Epoch end processes
            scheduler.step()
            eval_model(
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="chexpert",
        choices=["a", "chexpert", "mimic-cxr", "open-i"],
        help="Corpus name",
    )
    ...
    parser.add_argument(
        "--ignore-blank", default=False, action="store_true", help="Ignore blank labels"
    )
    parser.add_argument(
        "--img-no-augment",
        dest="img_augment",
    ...
    parser.add_argument(
        "--img-trans", type=str, default="pad", help="Image transformation mode"
    )
    parser.add_argument("--lr", type=float, default=1e-4, help="Learning rate")
    parser.add_argument(
        "--lr-gamma", type=float, default=0.1, help="A learning rate scheduler gamma"
    )
    parser.add_argument(
        "--lr-step", type=int, default=16, help="A learning rate scheduler step"
    )
