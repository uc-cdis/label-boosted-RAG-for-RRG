...
class ReportGenerationModel(nn.Module):
    ...
    def forward(
        ...
        image_targets: List[
            Dict
        ],  # contains a dict for every image with keys "boxes" and "labels"
        ...
        if self.training:
            ...
            obj_detector_loss_dict, top_region_features, class_detected = (
                self.object_detector(images, image_targets)
            )
            ...
            classifier_loss_region_selection = self.binary_classifier_region_selection(
                ...
                class_detected,
                return_loss=True,
                region_has_sentence=region_has_sentence,
            )
            ...
            valid_input_ids, valid_attention_mask, valid_region_features = (
                self.get_valid_decoder_input_for_training(
                    class_detected,
                    region_has_sentence,
                    input_ids,
                    attention_mask,
            ...
            obj_detector_loss_dict, detections, top_region_features, class_detected = (
                self.object_detector(images, image_targets)
            )
            ...
            ) = self.binary_classifier_region_selection(
                top_region_features,
                class_detected,
                return_loss=True,
                region_has_sentence=region_has_sentence,
            ...
            classifier_loss_region_abnormal, predicted_abnormal_regions = (
                self.binary_classifier_region_abnormal(
                    top_region_features, class_detected, region_is_abnormal
                )
            )
            ...
            valid_input_ids, valid_attention_mask = (
                self.get_valid_decoder_input_for_evaluation(
                    selected_regions, input_ids, attention_mask
                )
            )
    ...
    def get_valid_decoder_input_for_training(
        ...
        valid_input_ids = input_ids[
            valid_reshaped
        ]  # of shape [num_detected_regions_with_non_empty_gt_phrase_in_batch x seq_len]
        valid_attention_mask = attention_mask[
            valid_reshaped
        ]  # of shape [num_detected_regions_with_non_empty_gt_phrase_in_batch x seq_len]
        valid_region_features = region_features[
            valid
        ]  # of shape [num_detected_regions_with_non_empty_gt_phrase_in_batch x 1024]
    ...
    def get_valid_decoder_input_for_evaluation(
        ...
        valid_input_ids = input_ids[
            selected_regions
        ]  # of shape [num_regions_selected_in_batch x seq_len]
        valid_attention_mask = attention_mask[
            selected_regions
        ]  # of shape [num_regions_selected_in_batch x seq_len]
    ...
    def generate(
        ...
        _, detections, top_region_features, class_detected = self.object_detector(
            images
        )
        ...
        selected_regions, selected_region_features = (
            self.binary_classifier_region_selection(
