...
def main(args):
    ...
    with open("radnli_pseudo-train.jsonl", "w", encoding="utf-8") as out:
        with open(
            os.path.join("resources", "radnli_pseudo-train_indexes.jsonl"),
            encoding="utf-8",
        ) as f:
            for line in f:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Make RadNLI pseudo training data from MIMIC-CXR"
    )
    parser.add_argument(
        "sections", type=str, help="A path to the MIMIC-CXR sectioned file"
    )
