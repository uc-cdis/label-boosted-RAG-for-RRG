...
class _CaptioningData(data.Dataset):
    ...
    def _transform(cls, cache_image, resize_dim=256, mode="center", augment=False):
        ...
        if cache_image:
            if mode == cls.TRANSFORM_CENTER:
                return Compose([Resize(resize_dim), CenterCrop(224)]), Compose(
                    augs + [ToTensor(), norm]
                )
            elif mode == cls.TRANSFORM_PAD:
                return Compose([PadSquare(), Resize(224)]), Compose(
                    augs + [ToTensor(), norm]
                )
            else:
                raise ValueError("Unknown transform mode {0}".format(mode))
            ...
            if mode == cls.TRANSFORM_CENTER:
                return None, Compose(
                    [Resize(resize_dim), CenterCrop(224)] + augs + [ToTensor(), norm]
                )
            elif mode == cls.TRANSFORM_PAD:
                return None, Compose(
                    [PadSquare(), Resize(224)] + augs + [ToTensor(), norm]
                )
            else:
                raise ValueError("Unknown transform mode {0}".format(mode))
...
class _RadiologyReportData(_CaptioningData):
    ...
    def __init__(
        ...
        super(_RadiologyReportData, self).__init__(
            root,
            split,
            cache_image,
            cache_text,
...
class PretrainedEmbeddings:
    ...
    def load_embeddings(cls, path):
        ...
        word_idxs = {
            "__PAD__": cls.INDEX_PAD,
            "__START__": cls.INDEX_START,
            "__UNK__": cls.INDEX_UNKNOWN,
        ...
        try:
            ...
            for line in f:
                ...
                embed = np.array(
                    list(map(lambda v: float(v), entry[1:])), dtype="float32"
                )
    ...
    def __call__(self, texts):
        ...
        for text in texts:
            ...
            for sentence in filter(
                lambda sent: len(sent) > 0, self.sentsplitter.split(text)
            ):
                if len(sentence) > 0:
...
class ToWordIndexes:
    def __init__(
        ...
        self.textfilter = textfilter
        self.tokenfilter = tokenfilter
        self.to_tokenized_text = ToTokenizedTexts(
            sentsplitter, tokenizer, textfilter, tokenfilter
        )
    ...
    def __call__(self, text):
        ...
        if not self.pre_processed:
            sentences = list(
                filter(lambda sent: len(sent) > 0, self.sentsplitter.split(text))
            )
        else:
    ...
    def __repr__(self):
        return self.__class__.__name__ + "(size=({0},{1}))".format(
            self.max_sent, self.max_word
        )
