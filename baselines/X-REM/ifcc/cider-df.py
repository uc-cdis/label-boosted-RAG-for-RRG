...
def main(args):
    ...
    if args.corpus == "mimic-cxr":
        path = os.path.join(
            args.data, "mimic-cxr-resized", "2.0.0", MIMICCXRData.SPLITS_PATH
        )
        train_ids = {}
        ...
        path = os.path.join(
            args.data, "mimic-cxr-resized", "2.0.0", MIMICCXRData.SECTIONED_PATH
        )
        with gzip.open(path, "rt", encoding="utf-8") as f:
            header = f.readline()
            reader = csv.reader(f)
...
def parse_args():
    ...
    parser.add_argument(
        "--section", type=str, default="findings", help="Target section"
    )
