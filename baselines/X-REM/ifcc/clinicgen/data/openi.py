...
class OpenIData(_RadiologyReportData):
    ...
    def __init__(
        ...
        pre_transform, self.transform = OpenIData.get_transform(
            ...
            for i in range(1, total + 1):
                ...
                if os.path.exists(path):
                    ...
                    if split is None or (uid in splits and splits[uid] == split):
                        ...
                        findings = ""
                        for ele in rt.findall(
                            ".//AbstractText[@Label='{0}']".format("FINDINGS")
                        ):
                            findings = ele.text
                        ...
                        if len(findings) > 0 and len(image_ids) > 0:
                            for image_id in image_ids:
                                image = os.path.join(
                                    root,
                                    OpenIData.IMAGES_DIR,
                                    "{0}.png".format(image_id),
                                )
                                report = (
                                    self.extract_text(path, compress=True)
                                    if cache_text
                                    else path
                                )
                                if cache_image:
    ...
    def extract_section(self, text):
        ...
        if self.section is None:
            ...
            for ele in rt.findall(
                ".//AbstractText[@Label='{0}']".format(self.section.upper())
            ):
                report = ele.text
                if report is None:
                    report = ""
