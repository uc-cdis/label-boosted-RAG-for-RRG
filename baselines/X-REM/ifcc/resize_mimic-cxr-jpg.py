    ...
    if not os.path.exists(resized_dir):
        ...
        for item in os.listdir(os.path.join(args.jpgs, "2.0.0")):
            if item.endswith(".csv.gz"):
                shutil.copy(
                    os.path.join(args.jpgs, "2.0.0", item),
                    os.path.join(resized_dir, "2.0.0", item),
                )
    # Copy various MIMIC-CXR-JPG data
    shutil.copy(
        os.path.join(args.jpgs, "2.0.0", "mimic-cxr-2.0.0-chexpert.csv.gz"),
        os.path.join(resized_dir, "2.0.0"),
    )
    shutil.copy(
        os.path.join(args.jpgs, "2.0.0", "mimic-cxr-2.0.0-metadata.csv.gz"),
        os.path.join(resized_dir, "2.0.0"),
    )
    shutil.copy(
        os.path.join(args.jpgs, "2.0.0", "mimic-cxr-2.0.0-split.csv.gz"),
        os.path.join(resized_dir, "2.0.0"),
    )
    # Resize images
                    ...
                    for item3 in os.listdir(os.path.join(in_files, item1, item2)):
                        if item3.startswith("s"):
                            if not os.path.exists(
                                os.path.join(out_files, item1, item2, item3)
                            ):
                                os.mkdir(os.path.join(out_files, item1, item2, item3))
                            for item4 in os.listdir(
                                os.path.join(in_files, item1, item2, item3)
                            ):
                                if item4.endswith(".jpg"):
                                    ...
                                    if not os.path.exists(out_image_path):
                                        image_path = os.path.join(
                                            in_files, item1, item2, item3, item4
                                        )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Resize MIMIC-CXR-JPG jpgs to 256 pixels pngs"
    )
