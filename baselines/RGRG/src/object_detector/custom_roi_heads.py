...
class CustomRoIHeads(RoIHeads):
    ...
    def get_top_region_features_detections_class_detected(
        self, box_features, box_regression, class_logits, proposals, image_shapes
    ):
        """
        ...
        if self.return_feature_vectors:
            ...
            region_features_per_img = [
                None
            ] * num_images  # dummy list such that we can still zip everything up
        ...
        if not self.training:
            ...
            pred_region_boxes_per_img = torch.split(
                pred_region_boxes, boxes_per_image, dim=0
            )
        else:
            pred_region_boxes_per_img = [
                None
            ] * num_images  # dummy list such that we can still zip everything up
        ...
        output["class_detected"] = (
            []
        )  # list collects the bool arrays of shape [29] that specify if a class was detected (True) for each image
        output["top_region_features"] = (
            []
        )  # list collects the tensors of shape [29 x 2048] of the top region features for each image
        ...
        output["detections"] = {"top_region_boxes": [], "top_scores": []}

        for (
            pred_scores_img,
            pred_region_boxes_img,
            region_features_img,
            img_shape,
        ) in zip(
            pred_scores_per_img,
            ...
            mask_pred_classes = torch.nn.functional.one_hot(
                pred_classes, num_classes=29
            ).to(pred_scores_img.device)
            ...
            if not self.training:
                ...
                pred_region_boxes_img = box_ops.clip_boxes_to_image(
                    pred_region_boxes_img, img_shape
                )
                ...
                top_region_boxes = pred_region_boxes_img[
                    indices_with_top_scores,
                    torch.arange(
                        start=0,
                        end=29,
                        dtype=torch.int64,
                        device=indices_with_top_scores.device,
                    ),
                ]
        ...
        output["class_detected"] = torch.stack(
            output["class_detected"], dim=0
        )  # of shape [batch_size x 29]

        if self.return_feature_vectors:
            output["top_region_features"] = torch.stack(
                output["top_region_features"], dim=0
            )  # of shape [batch_size x 29 x 2048]

        if not self.training:
            output["detections"]["top_region_boxes"] = torch.stack(
                output["detections"]["top_region_boxes"], dim=0
            )  # of shape [batch_size x 29 x 4]
            output["detections"]["top_scores"] = torch.stack(
                output["detections"]["top_scores"], dim=0
            )  # of shape [batch_size x 29]
    ...
    def forward(
        ...
        if self.return_feature_vectors or not self.training:
            ...
            output = self.get_top_region_features_detections_class_detected(
                box_features, box_regression, class_logits, proposals, image_shapes
            )
            ...
            if self.return_feature_vectors:
                ...
                roi_heads_output["top_region_features"] = self.dim_reduction(
                    output["top_region_features"]
                )
