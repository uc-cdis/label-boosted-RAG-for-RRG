...
class _Image2Text(torch.nn.Module):
    ...
    def __init__(
        ...
        if rl_opts is not None:
            ...
            self.evaluator = GenEval(
                ...
                rouge=rl_opts.rouge,
                cider=rl_opts.cider,
                cider_df=rl_opts.cider_df,
                spice=rl_opts.spice,
                bert_score=rl_opts.bert_score,
                bert_score_penalty=rl_opts.bert_score_penalty,
                nli=rl_opts.nli,
                nli_compare=rl_opts.nli_compare,
                nli_label=rl_opts.nli_label,
                nli_neutral_score=rl_opts.nli_neutral_score,
                nli_prf=rl_opts.nli_prf,
                nli_batch=rl_opts.nli_batch,
                nli_cache=self.NLI_CACHE,
                entity_match=rl_opts.entity_match,
                entity_mode=rl_opts.entity_mode,
                nthreads=rl_opts.nthreads,
                pin_memory=rl_opts.pin_memory,
                sentsplitter="nltk",
                verbose=verbose,
            )
    ...
    def optimize(cls, name, epoch, image_finetune_epoch):
        if name == Optimizers.IMAGE:
            if (
                epoch is not None
                and image_finetune_epoch is not None
                and epoch < image_finetune_epoch
            ):
                return False
            else:
                return True
    ...
    def _decode_words_beam(
        ...
        if beam_size == 1:
            return self._decode_words_greedy(
                words, states, allow_stop=allow_stop, recover_words=recover_words
            )
        elif diversity_rate > 0.0:
            return self._decode_words_diverse(
                words, states, beam_size, recover_words, diversity_rate
            )

        logs = (
            {"word": [], "score": [], "score_current": [], "beam_buffer": []}
        ...
        for j in range(self.max_word):
            if stop_states.sum() <= 0 and allow_stop:
                ...
                state_buffer = list(
                    map(lambda sbuf: torch.stack(sbuf, dim=1), state_buffer)
                )
                ...
                rc_beam_buffer = (
                    self._beam_buffer(words.shape[0], beam_size)
                    if recover_words is not None
                    else None
                )
                    ...
                    if stop_states[m] != 0:
                        ...
                        if len(ei[0]) > 0:
                            ...
                            if torch.equal(eos_idx[0], zero_index):
                                ...
                                ps[m][eos_idx] = ps.new_full(
                                    (len(eos_idx),), self.EOS_SCORE
                                )
                        nwb_cpu = nwb.cpu()
                        if not stop:
                            ...
                            for current_idx in range(len(nwb)):
                                ...
                                new_beam_buffer[m, current_idx] = beam_buffer[
                                    m, prev_idx
                                ] + [w]
                                if recover_words is not None:
                                    rc_beam_buffer[m, current_idx] = (
                                        recover_words.array(
                                            beam_buffer[m, prev_idx] + [w]
                                        )
                                    )
                        else:
                        ...
                        for current_idx in range(len(nwb)):
                            ...
                            if recover_words is not None:
                                rc_beam_buffer[m, current_idx] = recover_words.array(
                                    beam_buffer[m, current_idx] + [w]
                                )
        ...
        beam_words = np.zeros(
            (beam_buffer.shape[0], beam_buffer.shape[1], self.max_word), dtype="long"
        )
    ...
    def _decode_words_diverse(
        self, words, states, beam_size, recover_words=None, diversity_rate=0.0
    ):
        logs = (
            {"word": [], "score": [], "score_current": [], "beam_buffer": []}
            if recover_words is not None
            ...
            for m in range(next_word_buffer.shape[0]):
                ...
                if len(ei[0]) > 0:
                    ...
                    if torch.equal(eos_idx[0], zero_index):
                        ...
                        beam_results[m].append(
                            beam_buffer[m, prev_idx] + [int(nwb[eos_idx[0]])]
                        )
                    # If top-B includes EOS, force EOS to be not selected in the next iteration.
        ...
        for m in range(beam_buffer.shape[0]):
            for k in range(beam_buffer.shape[1]):
                ...
                if rs_len - 1 >= k:
                    ...
                    beam_words[m, k] = np.array(
                        beam_buffer[m, k - rs_len], dtype="long"
                    )
        return torch.tensor(beam_words), logs
    ...
    def _decode_words_greedy(self, words, states, allow_stop=True, recover_words=None):
        ...
        words = words[:, 0]
        stop_states = np.ones(words.shape[0], dtype="uint8")
        greedy_buffer = torch.full(
            (words.shape[0], 1, self.max_word),
            PretrainedEmbeddings.INDEX_PAD,
            dtype=torch.long,
        )
        ...
        for j in range(self.max_word):
            if stop_states.sum() <= 0 and allow_stop:
                # Skip the word generation since all generation are stopped
                greedy_buffer[:, 0, j] = torch.full(
                    (words.shape[0],), PretrainedEmbeddings.INDEX_PAD, dtype=torch.long
                )
            else:
                # proc_word should be overridden in subclasses
                ...
                stop_states *= (words_np != PretrainedEmbeddings.INDEX_EOS).astype(
                    "uint8"
                )
    ...
    def _sample_words(self, word, states, nucleus_p=None, allow_stop=True):
        if nucleus_p is not None:
            return self._sample_words_nucleus(
                word, states, pthresh=nucleus_p, allow_stop=allow_stop
            )
        words, lprobs = [], []
        stop_states = None
        for j in range(self.max_word):
            if stop_states is not None and stop_states.sum() <= 0 and allow_stop:
                ...
                if stop_states is None:
                    stop_states = (word_np != PretrainedEmbeddings.INDEX_EOS).astype(
                        "uint8"
                    )
                else:
                    stop_states *= (word_np != PretrainedEmbeddings.INDEX_EOS).astype(
    ...
    def deflatten_image(self, x):
        if self.multi_image > 1:
            if len(x.shape) == 2:
                x = x.view(
                    int(x.shape[0] / self.multi_image), self.multi_image, x.shape[1]
                )
            elif len(x.shape) == 3:
                x = x.view(
                    int(x.shape[0] / self.multi_image),
                    self.multi_image,
                    x.shape[1],
                    x.shape[2],
                )
    ...
    def self_critical_reward(
        self, gens_sample, gens_greedy, references, masks_sample, log_probs, ids=None
    ):
        """
        ...
        num_words = (
            masks_sample.sum(axis=(1, 2))
            if len(masks_sample.shape) == 3
            else masks_sample.sum(axis=1)
        )
        ...
        for i in range(len(gens_greedy)):
            ...
            rids.append(ids[i])
        _, scores_detailed = self.evaluator.eval(
            hids, refs, hypos, self.rl_opts.tfidf, ref_ids=rids
        )
        # Rewards and losses
        log_probs *= log_probs.new_tensor(masks_sample)
        # Calculate mean log word probabilities considering pads
        log_probs = (
            log_probs.sum(axis=(1, 2))
            if len(masks_sample.shape) == 3
            else log_probs.sum(axis=1)
        )
    ...
    def train_step(
        ...
        for step, weight in steps:
            loss, loss_reward_vals = step(
                encoded_data, targ, device, non_blocking, ids=ids
            )
...
class StepTFR(object):
    ...
    def get_tfr(self):
        if self.step_size is not None:
            tfr = max(
                self.base_tfr - self.gamma * (self.last_epoch // self.step_size),
                self.MIN_TEACHER_FORCING_RATE,
            )
            return tfr
        else:
