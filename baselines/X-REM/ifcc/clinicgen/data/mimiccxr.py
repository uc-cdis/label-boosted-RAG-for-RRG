...
class MIMICCXRData(_RadiologyReportData):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            single_image_doc=single_image_doc,
            dump_dir=dump_dir,
        )
        pre_transform, self.transform = MIMICCXRData.get_transform(
            cache_image, img_mode, img_augment
        )
        self.target_transform = target_transform
        self.chexpert_labels_path = os.path.join(
            root, "mimic-cxr-jpg", "2.0.0", self.CHEXPERT_PATH
        )

        self.view_positions = {}
        doc_image_map = {}
        with gzip.open(
            os.path.join(root, "mimic-cxr-resized", "2.0.0", self.META_PATH),
            "rt",
            encoding="utf-8",
        ) as f:
            f.readline()
        ...
        with gzip.open(
            os.path.join(root, "mimic-cxr-resized", "2.0.0", self.SECTIONED_PATH),
            "rt",
            encoding="utf-8",
        ) as f:
        ...
        with gzip.open(
            os.path.join(root, "mimic-cxr-resized", "2.0.0", self.SPLITS_PATH),
            "rt",
            encoding="utf-8",
        ) as f:
            f.readline()
                    ...
                    if split is None or split == row[3]:
                        ...
                        if cache_text:
                            ...
                            report = (
                                sections[sid]
                                if sid in sections
                                else gzip.compress(pickle.dumps({}))
                            )
    ...
    def __getitem__(self, index):
        ...
        if self.multi_image > 1:
            vp = [
                self.view_position_embedding(self.view_positions[iid])
                for iid in self.image_ids[index]
            ]
