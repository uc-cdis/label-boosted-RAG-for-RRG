...
class MeshedTransformerMaxDecoderLayer(TransformerDecoderLayer):
    def __init__(
        ...
        super(MeshedTransformerMaxDecoderLayer, self).__init__(
...
class MultiheadAttentionWithMem(MultiheadAttention):
    def __init__(
        ...
        super(MultiheadAttentionWithMem, self).__init__(
    ...
    def multi_head_attention_forward(
        ...
        scaled_mem_k = ((embed_dim // num_heads) ** 0.5) * mem_k.expand(
            num_memory, k.shape[1], k.shape[2]
        )
        scaled_mem_v = (num_memory**0.5) * mem_v.expand(
            num_memory, v.shape[1], v.shape[2]
        )
...
class TransformerEncoderLayerWithMem(Module):
    def __init__(
        self, d_model, nhead, nmem, dim_feedforward=2048, dropout=0.1, activation="relu"
    ):
        super(TransformerEncoderLayerWithMem, self).__init__()
        self.self_attn = MultiheadAttentionWithMem(
            d_model, nhead, nmem, dropout=dropout
        )
...
class M2Transformer(_TransformerCaptioner):
    def __init__(
        ...
        # Transformer Encoder
        encoder_layer = TransformerEncoderLayerWithMem(
            feat_dim, nhead=8, nmem=num_memory
        )
        self.encoder = MeshedTransformerEncoder(
    ...
    def decode_beam(
        ...
        return super(M2Transformer, self).decode_beam(
