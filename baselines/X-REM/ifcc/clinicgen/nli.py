...
class _NLIScorer:
    ...
    def sentence_scores(
        ...
        if compare == self.COMPARE_ALL:
            ...
            result = self.sentence_scores_bert_score(
                texts1, texts2, label, thresh=self.THRESH_FIX, prf=prf
            )
        ...
        if self.verbose:
            print(
                "NLI {0} pairs with gpu={1}: {2}s".format(
                    len(texts1), self.gpu, time.time() - t
                )
            )
        return result
                ...
                for i in range(len(rp_hypos)):
                    ...
                    for j in range(len(rp_prems)):
                        if prf == "h":
                            ...
                            tlabel = (
                                label if label != self.LABEL_ALL else self.LABEL_ENTAIL
                            )
    ...
    def sentence_scores_bert_score(
        self, texts1, texts2, label="entailment", thresh="none", prf="f"
    ):
        ...
        for tid in tids:
            for k, quad in enumerate(
                [
                    (scores1, tsents1, tsents2, thresh1),
                    (scores2, tsents2, tsents1, thresh2),
                ]
            ):
                ...
                for i in range(len(scores[tid])):
                    if len(scores[tid][i]) > 0:
                        ...
                        if self.cache is not None and (prem, hypo) in self.cache:
                            ...
                            if scores[tid][i][fidx] >= thresh and (
                                (k == 0 and prf != "p") or (k == 1 and prf != "r")
                            ):
                                tids_all.append(tid)
            ...
            for k, num_sent in zip([0, 1], (len(tsents1[tid]), len(tsents2[tid]))):
                for i in range(num_sent):
                    if i in cache_sents[k][tid]:
                        ...
                        i in valid_sents[k][tid] and valid_sents[k][tid][i] is not None
                    ):
                    ...
                    if prf == self.METRIC_FB1_HARD:
                        ...
                        if prob is not None:
                            ...
                            if label == self.LABEL_ALL:
                                prob = {
                                    self.LABEL_ENTAIL: -1.0,
                                    self.LABEL_NEUTRAL: -1.0,
                                    self.LABEL_CONTRADICT: -1.0,
                                }
                            else:
                                prob = -1.0
                    else:
                            ...
                            if prob is not None:
                                ...
                                if label == self.LABEL_ENTAIL:
                                    if (
                                        ...
                                        and neutral_prob > prob_l
                                        and neutral_prob > prob[self.LABEL_CONTRADICT]
                                    ):
                                        prob_l = -1.0
    ...
    def sentence_scores_tfidf(
        self, texts1, texts2, label="entailment", vectorizer=None
    ):
        prs, rcs, fb1s, stats = [], [], [], []
                ...
                for k, sent_len in zip([0, 1], [sent_len1, sent_len2]):
                    for i in range(sent_len):
                        if label == self.LABEL_ALL:
                            ...
                            if (
                                neutral_prob > prob
                                and neutral_prob > probs[idx][self.LABEL_CONTRADICT]
                            ):
                                prob = -1.0
                                rc_pr[k].append(0.5)
                            else:
    ...
    def score(self, cands, refs):
        ...
        if self.penalty:
            for idx, (cand, ref) in enumerate(zip(cands, refs)):
                ...
                penalty = np.e ** (
                    -((len(toks1) - len(toks2)) ** 2) / (2 * self.PENALTY_SIGMA**2)
                )
                out[-1][idx] *= penalty
...
class SimpleNLI(_NLIScorer):
    ...
    def load_model(cls, states=None):
        ...
        bertnli = BERTNLI(
            name, bert_type=bert_type, length=384, force_lowercase=True, device="cpu"
        )
        ...
        with torch.no_grad():
            for b1, b2 in batches:
                ...
                for i, idx in enumerate(idxs):
                    ...
                    probs.append(
                        {
                            "entailment": float(out[i][BERTNLI.LABEL_ENTAILMENT]),
