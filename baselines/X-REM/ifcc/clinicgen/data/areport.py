...
class AReportData(_RadiologyReportData):
    ...
    def __init__(
        ...
        pre_transform, self.transform = AReportData.get_transform(
        ...
        if meta is not None:
            self.chexpert_labels_path = os.path.join(
                os.path.dirname(meta), self.CHEXPERT_PATH
            )
            ...
            for rid in sorted(paths.keys()):
                for image in paths[rid]:
                    if rid not in exclude_dic:
                        report = os.path.join(
                            root, self.DIR_REPORTS, rid[0:2], rid[2:4], rid + ".txt"
                        )
                        if cache_text:
    ...
    def update_image_path(cls, root, image):
        ...
        for i, d in enumerate(image_dirs):
            if d == check_dir:
                image_i = i
        return os.path.join(
            "/".join(root_dirs[:check_i]), "/".join(image_dirs[image_i:])
        )
    ...
    def extract_section(self, text):
        ...
        if self.sub_section is not None:
            lines = list(
                filter(
                    lambda ln: ln.lower().startswith(self.sub_section.lower()), lines
                )
            )
        return "\n".join(lines)
    ...
    def filter_with_dic(self, dic, label="anatomy", print_num=True):
        ...
        for i in range(len(self.samples)):
            ...
            if (label == "anatomy" and rid not in dic) or (
                label == "validity" and rid in dic
            ):
