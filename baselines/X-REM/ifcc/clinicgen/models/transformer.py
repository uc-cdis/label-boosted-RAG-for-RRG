...
class _TransformerCaptioner(_Image2Text):
    ...
    def __init__(
        ...
        super(_TransformerCaptioner, self).__init__(
            max_word,
            multi_image,
            None,
            teacher_forcing,
    ...
    def _rl_step(self, encoded_data, targ, device, non_blocking, ids=None):
        ...
        with torch.no_grad():
            ...
            gens_greedy, _ = self.evaluator.recover_words(
                words_greedy.squeeze(dim=1).squeeze(dim=1)
            )
        self.train()
    ...
    def decode_beam(
        ...
        w = v.new_full(
            (v.shape[0], 1), PretrainedEmbeddings.INDEX_START, dtype=torch.long
        )
        states = (torch.tensor([0]), None, v)
        # Decode words
        beam_words, logs = self._decode_words_beam(
            w, states, beam_size, allow_stop, recover_words, diversity_rate
        )
    ...
    def decode_teacher_forcing(self, y, v):
        ...
        if self.teacher_forcing is None:
            ...
            tgt_mask = wt.new_full(
                (wt.shape[1], wt.shape[1]), float("-inf"), dtype=torch.float
            ).triu(1)
    ...
    def loss_nll(self, y, words):
        ...
        loss_word = cross_entropy(
            words, y, ignore_index=PretrainedEmbeddings.INDEX_PAD, reduction="mean"
        )
        return loss_word
...
class TransformerSimpleCaptioner(_TransformerCaptioner):
    def __init__(
        ...
        super(TransformerSimpleCaptioner, self).__init__(
            embeddings,
            feat_dim,
            max_word,
            multi_image,
...
class TransformerCaptioner(TransformerSimpleCaptioner):
    ...
    def decode_beam(
        ...
        return super(TransformerCaptioner, self).decode_beam(
            encoded_data, beam_size, allow_stop, recover_words, diversity_rate
        )
