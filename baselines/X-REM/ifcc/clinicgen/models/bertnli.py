...
class BERTNLI(torch.nn.Module):
    ...
    def forward(self, sent1s, sent2s):
        ...
        for idx, tokens in enumerate(buffer):
            ...
            attn_mask.append(
                torch.tensor([1 if token != "[PAD]" else 0 for token in tokens])
            )
        ...
        if seg_ids is not None:
            ...
            token_ids, attn_mask, seg_ids = data_cuda(
                token_ids, attn_mask, seg_ids, device=self.device
            )
        else:
        ...
        if self.bert_type == "distilbert":
            ...
            reps, cls = self.bert(
                token_ids, attention_mask=attn_mask, token_type_ids=seg_ids
            )
            if self.cls == "token":
                reps = reps[:, 0]
                reps = self.dropout(reps)
