...
from clinicgen.models.image2text import PretrainedEmbeddings, _Image2Text
from clinicgen.utils import data_cuda
from torch import sigmoid, tanh
from torch.distributions.categorical import Categorical
from torch.nn import LSTM, Dropout, Embedding, Linear, LSTMCell
...
class CNNRNNRNN(_Image2Text):
    ...
    def __init__(
        ...
        super(CNNRNNRNN, self).__init__(
            max_word,
            multi_image,
            multi_merge,
            teacher_forcing,
            image_finetune_epoch,
            rl_opts,
            word_idxs,
            verbose,
        )
        ...
        self.embeddings = Embedding.from_pretrained(
            embeddings, freeze=False, padding_idx=PretrainedEmbeddings.INDEX_PAD
        )
        self.word_dense = Linear(feat_dim, embeddings.shape[0], bias=False)

    def _decode_loop_word(
        self, vl, vg, t, beam_size, allow_stop, recover_words=None, diversity_rate=0.0
    ):
        if self.topic_as_state:
            hw, mw = t, t
        else:
            hw, mw = vg.new_zeros(vg.shape[0], self.feat_dim), vg.new_zeros(
                vg.shape[0], self.feat_dim
            )
        ws = vg.new_full(
            (vg.shape[0], 1), PretrainedEmbeddings.INDEX_START, dtype=torch.long
        )
        states = (t, vg, vl, hw, mw)
        sent_words, logs = self._decode_words_beam(
            ws, states, beam_size, allow_stop, recover_words, diversity_rate
        )
    ...
    def _loop_word(self, vl, vg, t, yw, mask, not_masked):
        ...
        for j in range(self.max_word):
            if torch.equal(mask[j], not_masked):
                ...
                sent_words.append(p)
                if (
                    self.teacher_forcing is None
                    or self.teacher_forcing.get_tfr() >= random.random()
                ):
                    w = yw[j]
                else:
                    p = softmax(p, dim=-1)
                ...
                p = (
                    vg.new_ones(vg.shape[0], self.embeddings.num_embeddings)
    ...
    def _rl_step(self, encoded_data, targ, device, non_blocking, ids=None):
        ...
        with torch.no_grad():
            ...
            gens_greedy, _ = self.evaluator.recover_words(
                stops_greedy, words_greedy.squeeze(dim=2)
            )
        self.train()
        ...
        rewards, loss_acc = self.self_critical_reward(
            gens_sample, gens_greedy, gens_ref, masks_sample, log_probs, ids=ids
        )
    ...
    def decode_beam(
        ...
        if self.parallel_sent:
            ...
            hs, ms = vg.new_zeros((1, batch_size, self.feat_dim)), vg.new_zeros(
                (1, batch_size, self.feat_dim)
            )
            ...
            vl = (
                vl.repeat(self.max_sent, 1, 1, 1)
                if self.multi_image > 1
                else vl.repeat(self.max_sent, 1, 1)
            )
            ...
            # Decodes words
            words, logs_sent = self._decode_loop_word(
                vl, vg, t, beam_size, allow_stop, recover_words, diversity_rate
            )
            ...
            stops, words = [], []
            hs, ms = vg.new_zeros((batch_size, self.feat_dim)), vg.new_zeros(
            ...
            for i in range(self.max_sent):
                ...
                if torch.equal(stop_state, batch_stop) and allow_stop:
                    words.append(
                        torch.zeros(
                            (batch_size, beam_size, self.max_word), dtype=torch.long
                        )
                    )
                else:
                    # Process each word
                    sent_words, logs_sent = self._decode_loop_word(
                        vl, vg, t, beam_size, allow_stop, recover_words, diversity_rate
                    )
    ...
    def decode_teacher_forcing(self, y, vl, vg):
        ...
        if self.parallel_sent:
            ...
            for i in range(self.max_sent):
                ...
                # Process each word
                y2 = y.permute(1, 2, 0)
                sent_words = self._loop_word(
                    vl, vg, t, y2[i], batch_mask_words[i], not_masked
                )
                words.append(sent_words)
            stops = torch.stack(stops, dim=1)
            words = torch.stack(words, dim=1)
    ...
    def loss_nll(self, y, stops, words):
        ...
        loss_word = cross_entropy(
            words.permute(0, 3, 1, 2),
            y,
            ignore_index=PretrainedEmbeddings.INDEX_PAD,
            reduction="mean",
        )
    ...
    def visual_sentinel(self, w, t, vg, vl, hw, mw):
        ...
        xw = (
            torch.cat((vg, e), dim=1)
            if self.topic_as_state
            else torch.cat((vg, t, e), dim=1)
        )
    ...
    def visual_sentinel_attention(self, vl, hw, z, s):
        ...
        b = (
            a2[:, :, -1].unsqueeze(dim=-1)
            if self.multi_image > 1
            else a2[:, -1].unsqueeze(dim=-1)
        )
